...
function injectDownloadButtons(container, filePath, rawHtml, parsedScripts, parsedStyles, parsedCdns) {
  ...
  if (isComponent) {
    ...
    sections.forEach((section, i) => {
      // Crear botón discreto de copiar variante
      const copyVHtml = `
      ...
      // Agregar clase group al section y poner botón al lado del h2
      section.classList.add('group');
      const h2 = section.querySelector('h2');
      if (h2) {
        h2.parentNode.insertBefore(copyVBtn, h2.nextSibling);
        // Hacer que h2 y botón estén en flex
